...
def render_quilt_image(colormap, seed, dpi, num=14, n_shapes=2000, figlen=figlen):
    ...
    for inum in range(num):
        ...
        for i in rng:
            ...
            c = colormap(abs(np.sin(2*np.pi*(i/tau+phasecol[inum]))))
...
original_cmap = matplotlib.colormaps['Greys']
opposite_cmap = matplotlib.colormaps['Greys_r']
final_cmap = plt.cm.turbo

# Render the first set
axis_image1 = render_quilt_image(colormap=original_cmap, seed=2183, dpi=dpi)
# Render the second set
axis_image2 = render_quilt_image(colormap=opposite_cmap, seed=192, dpi=dpi)

axis_image3 = render_quilt_image(colormap=final_cmap, seed=592, dpi=dpi)
...
pixlen = dpi*figlen
order = len(str(int(pixlen)))
coeff = 2 / (10**order)

color_applicator = lambda x,y: np.exp(-coeff*np.sqrt((x-pixlen/2)**2 + (y-pixlen/2)**2))*np.abs(np.sin(np.sqrt((x-pixlen/2)**2 + (y-pixlen/2)**2)))
...
filename = unique_filename( os.path.join(dirname, 'blackwhitecolor.png'))
# Display or save
