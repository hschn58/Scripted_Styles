...
def render_quilt_image(colormap, seed, dpi, num=14, n_shapes=4000, figlen=figlen, other = plt.cm.turbo):
    """
    ...
    x_base, y_top_base, y_bot_base = base_shape(num_points=200)
    freq = 4
    tau = n_shapes // freq
    ...
    for inum in range(num):
        ...
        for i in rng:
            ...
            c = colormap(coeff*abs(np.sin(2*np.pi*(i/tau+phasecol[inum]))) + base)
...
for wi in range(witer):
    for hi in range(hiter):
        ...
        if (pattern(wi, hi) > 0) and (pattern(wi, hi) <= param):
            axis_image_product[wi, hi, :] = axis_image1[wi, hi]

        elif (pattern(wi, hi) < 0) and (pattern(wi, hi) >= -param):
...
filename = unique_filename( os.path.join(dirname, f'{param} '+'3quilt_buy.jpg'))
# Display or save
