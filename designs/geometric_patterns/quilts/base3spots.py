...
axis_image3 = render_quilt_image(colormap=original_cmap, seed=2183, dpi=dpi)
...
for wi in range(w):
    for hi in range(h):
        ...
        if (pattern(wi, hi) > 1) and (pattern(wi, hi) < param):
            axis_image_product[wi, hi, :] = axis_image2[wi, hi]

        elif (pattern(wi, hi) < -1) and (pattern(wi, hi) > -param):
