...
def render_quilt_image(colormap, seed, dpi, num=14, n_shapes=2000, figlen=figlen, th_cmap = matplotlib.colormaps['magma']):
    """
    ...
    if colormap == th_cmap:
        form = lambda tau,phasecol,inum,i: 0.35*abs(np.sin(2*np.pi*(i/tau+phasecol[inum])))+ 0.2
    else:
        form = lambda tau,phasecol,inum,i: 0.7*abs(np.sin(2*np.pi*(i/tau+phasecol[inum]))) + 0.3
    ...
    for inum in range(num):
        ...
        for i in rng:
            ...
            c = colormap(form(tau,phasecol,inum,i))
...
th_cmap = matplotlib.colormaps['magma']
...
#axis_image4 = render_quilt_image(colormap=th_cmap, seed=2183, dpi=dpi)
...
pattern = lambda x,y: np.sin(x)*np.cos(y) + np.cos(y)
...
postpattern = lambda x,y: np.sin(y+(np.pi/2))*np.cos(x+(np.pi/2)) + np.cos(x+(np.pi/2))
...
for wi in range(witer):
    for hi in range(hiter):
        ...
        if (postpattern(x, y) > 0) and (postpattern(x, y) <= param):
            axis_image_product[wi, hi, :] = axis_image2[wi, hi]

        elif (postpattern(x, y) > param):
            axis_image_product[wi, hi, :] = axis_image1[wi,hi]
...
filename = unique_filename( os.path.join(dirname, 'plane_windows4.png'))
# Display or save
