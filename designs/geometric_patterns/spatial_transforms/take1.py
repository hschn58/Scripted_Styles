...
def base_shape(num_points=200):
    ...
    """
    x = np.linspace(-1.1, 1.1, num_points)
    y = np.exp(-x**2)
    return x, y, -y
...
for i in range(n_shapes):
    ...
    offset_x = R*1.2 * np.cos(freq*2*np.pi * (i / n_shapes)) +R*np.cos((i/n_shapes)*2*np.pi +np.pi/3) +R*0.5*np.cos((i/n_shapes)*400*np.sqrt(2)*2*np.pi)
    offset_y = R*1.2 * np.sin(freq*2*np.pi * (i / n_shapes)) +R*np.sin(2*(i/n_shapes)*np.pi +np.pi/3) +R*0.5*np.sin((i/n_shapes)*400*np.sqrt(2)*2*np.pi)
    scale_x  = 1.0 + 0.5 * (i / n_shapes)
    scale_y  = 1.0 - 0.5 * (i / n_shapes)
    rotation_deg = 45*(i/n_shapes)
    ...
    ax.fill_between(x_top, y_top, y_bot, color=my_cmap(0.8*abs(np.sin(100*np.pi*(i/tau)))+0.1), alpha=1, linewidth=0)
