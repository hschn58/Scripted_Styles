...
for inum in range(num):
    ...
    sections=[0.3+x*(0.7/4) for x in range(4)]
    ...
    for i in rng:
        ...
        if i/tau <1:
            ax.fill_between(x_top, y_top, y_bot, color=my_cmap((0.7/4)*abs(np.sin(2*np.pi*(i/tau+phasecol[inum])))+sections[0]), alpha=1, linewidth=0)
        elif (i/tau <2) and (i/tau >=1):
            ax.fill_between(x_top, y_top, y_bot, color=my_cmap((0.7/4)*abs(np.sin(2*np.pi*(i/tau+phasecol[inum])))+sections[1]), alpha=1, linewidth=0)
        elif (i/tau <3) and (i/tau >=2):
            ax.fill_between(x_top, y_top, y_bot, color=my_cmap((0.7/4)*abs(np.sin(2*np.pi*(i/tau+phasecol[inum])))+sections[2]), alpha=1, linewidth=0)
        else:
            ax.fill_between(x_top, y_top, y_bot, color=my_cmap((0.7/4)*abs(np.sin(2*np.pi*(i/tau+phasecol[inum])))+sections[3]), alpha=1, linewidth=0)
