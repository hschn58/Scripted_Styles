...
for inum in range(num):
    ...
    for i in rng:
        ...
        #plus inward rotation

        # Define how the offset, scaling, and rotation vary with i
        offset_x = 3*(i/n_shapes) +2*(i/n_shapes)
        offset_y = 0.5* np.sin(2*2*np.pi * (i / n_shapes +phasephs[inum])) +num - 4*(i/n_shapes) 
        scale_x  = 0.75 + 0.25*np.sin(6*np.pi*(i/tau+phasecol[inum])) - 0.25*(i/n_shapes)
        scale_y  = 1.0 + 0.25*np.sin(6*np.pi*(i/tau+phasecol[inum])) + 0.25*(i/n_shapes)
        rotation_deg = 360*np.sin(6*np.pi*(i/tau+phasecol[inum]))     + 5*(i/n_shapes)
        ...
        plt.plot(x, y, color=my_cmap(0.5*abs(np.sin(2*np.pi*(i/tau+phasecol[inum])))+0.5), linewidth=1)
...
plt.savefig(os.path.dirname(os.path.abspath(__file__))+'/wrap.jpg', transparent=True, dpi=1200, bbox_inches='tight', pad_inches=0.0)
